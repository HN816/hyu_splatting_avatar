...
class GaussianBase(nn.Module):
    ...
    # render
    def render_to_camera(self, viewpoint_cam, pipe, background='random', scaling_modifer=1.0):
        if background == 'white':
            background = torch.tensor([1, 1, 1], dtype=torch.float16, device='cuda')
        elif background == 'black':
            background = torch.tensor([0, 0, 0], dtype=torch.float16, device='cuda')
