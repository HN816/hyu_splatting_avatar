...
def read():
    global conn
    messageLengthBytes = read_exactly(4)
    if not messageLengthBytes or len(messageLengthBytes) < 4:
        print("[read] messageLength incomplete or socket closed")
        return {}

    messageLength = int.from_bytes(messageLengthBytes, 'little')
    messageBytes = read_exactly(messageLength)
    if not messageBytes or len(messageBytes) < messageLength:
        print(f"[read] message incomplete: expected {messageLength}, got {len(messageBytes) if messageBytes else 0}")
        return {}

    print(f"[read] message received: {messageBytes[:50]}...")  # 앞 50바이트 출력
    return json.loads(messageBytes.decode("utf-8"))
...
def receive():
    message = read()
    width = message.get("resolution_x", 0)
    height = message.get("resolution_y", 0)
...
if __name__ == '__main__':
    ...
    # TCP 서버 루프 (연결만 유지, 실제 데이터 통신은 호출 함수에서 처리)
