...
def render_frame():
    global gs_model, viewpoint_cam, pipe
    with torch.no_grad():
        render_pkg = gs_model.render_to_camera(viewpoint_cam, pipe, background='white', scaling_modifer=0.0001)
...
def frame_producer():
    ...
    for idx in tqdm(itertools.count(), desc="Streaming frames",mininterval=1.0):
