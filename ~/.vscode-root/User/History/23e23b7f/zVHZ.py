...
def render_frame():
    ...
    with torch.no_grad():
        ...
        render_pkg = gs_model.render_to_camera(viewpoint_cam, pipe, background='white', render_depth=False, render_normal=False, scaling_modifer=0.05)

    if "depth" in render_pkg: del render_pkg["depth"]
    if "alpha" in render_pkg: del render_pkg["alpha"]
    ...
    while True:
        with frame_lock:
            if current_frame_jpeg is not None:
                socketio.emit('frame', current_frame_jpeg, to=None)
        socketio.sleep(1/30)
